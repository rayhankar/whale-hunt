...
# ================= 1. FUTURES BOT SINIFI (BULUT UYUMLU) =================
class FuturesCloudBot:
    def __init__(self):
        ...
        if 'bot_state' not in st.session_state:
            st.session_state.bot_state = {
                ...
                'portfolio': {}, 
                'logs': [f"[{datetime.now().strftime('%H:%M')}] ☁️ Futures Cloud Başlatıldı. Bakiye: {self.initial_balance}$"]
            }
    ...
    def get_total_equity(self, current_prices):
        """Toplam Varlık (Nakit + Marjin + PnL)"""
        ...
        for sym, dat in state['portfolio'].items():
            ...
            # PnL Tutar = Pozisyon Büyüklüğü * Değişim
    ...
    def add_margin(self, symbol, current_price):
        ...
        if state['balance'] < add_amount:
            self.log(f"⚠️ Yetersiz Bakiye: {symbol} ekleme yapılamadı.")
            return False
        ...
        for symbol, data in portfolio.items():
            ...
            # 2. Ekleme veya Stop
                ...
                if self.margin_mode and data['adds_count'] < self.max_adds:
                    ...
                    continue # Ekleme yapıldı, stopu atla
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("🔥 Kaldıraç & Risk")
    bot.leverage = st.slider("Kaldıraç (x)", 1, 20, 10)
    bot.trade_pct = st.slider("İşlem Başı Bakiye (%)", 1, 20, 5)
    
    st.divider()
    st.subheader("🛡️ Kurtarma (Marjin Ekleme)")
    bot.margin_mode = st.checkbox("Ekleme Modu Açık", value=True)
    bot.margin_trigger = st.number_input("Ekleme Tetikleyicisi (Zarar %)", value=15.0)
...
if not df.empty:
    ...
    with metrics_container.container():
        ...
        c3.metric("GELECEK İŞLEM GÜCÜ", f"${power:.0f}", help=f"${margin:.0f} Marjin x {bot.leverage}")

    # 2. İÇERİK
    with content_container.container():
        tab1, tab2, tab3 = st.tabs(["📊 PİYASA", "💼 POZİSYONLAR", "📜 LOGLAR"])
            ...
            if state['portfolio']:
                ...
                for sym, dat in list(state['portfolio'].items()):
                    ...
                    with st.container():
                        ...
                        cc[5].caption(f"Ekleme: {dat['adds_count']}/{bot.max_adds}")
    ...
    st.warning("Veri bekleniyor... (API)")

# Bulut Yenileme Mekanizması
if bot_active:

...
# ================= SAYFA AYARLARI =================
st.set_page_config(page_title="AI Futures Bot", page_icon="🎰", layout="wide")
...
<style>
    ...
    .bot-log { font-family: 'Courier New', monospace; font-size: 12px; color: #00ff00; background-color: #000; padding: 10px; border-radius: 5px; height: 300px; overflow-y: scroll; }
...
class FuturesTradingBot:
    def __init__(self):
        ...
        if 'bot_state' not in st.session_state:
            st.session_state.bot_state = {
                ...
                'portfolio': {}, 
                'logs': [f"[{datetime.now().strftime('%H:%M:%S')}] 🎰 Futures Bot Hazır. Bakiye: {self.initial_balance}$"]
            }
    ...
    def get_total_equity(self, current_prices):
        """Toplam Varlık (Nakit + Unrealized PnL + Teminat)"""
    ...
    def add_margin(self, symbol, current_price, current_pnl_pct):
        state = st.session_state.bot_state
        data = state['portfolio'][symbol]
        
        if data['adds_count'] >= self.max_adds:
            return False 
            
        add_amount = data['margin'] * (self.add_amount_pct / 100.0)
        
        if state['balance'] < add_amount:
            self.log(f"⚠️ EKLEME YAPILAMADI: Yetersiz Bakiye ({symbol})")
            return False

        old_size_coins = data['amount_coins']
        new_purchasing_power = add_amount * self.leverage
        new_coins = new_purchasing_power / current_price
        
        total_coins = old_size_coins + new_coins
        new_entry = ((data['entry'] * old_size_coins) + (current_price * new_coins)) / total_coins
        ...
        self.log(f"♻️ MARJİN EKLENDİ ({data['adds_count']}/{self.max_adds}): {symbol} | Yeni Ort: {new_entry:.4f}")
        return True
    ...
    def manual_sell(self, symbol, current_price):
        ...
        if symbol in state['portfolio']:
            ...
            pnl = position_size - entry_value
            
            return_amount = data['margin'] + pnl
            state['balance'] += return_amount
            
            pnl_pct = (pnl / data['margin']) * 100
            del state['portfolio'][symbol]
            
            self.log(f"MANUEL KAPATMA: {symbol} | PnL: ${pnl:.2f} (%{pnl_pct:.2f})")
            return True
        return False
    ...
    def check_portfolio(self, current_prices):
        ...
        for symbol, data in portfolio.items():
            ...
            leveraged_pnl_pct = raw_change * self.leverage * 100
            
            sell = False
            reason = ""
            
            if leveraged_pnl_pct >= self.take_profit:
                sell = True; reason = f"✅ TP ALINDI (%{leveraged_pnl_pct:.2f})"
            elif leveraged_pnl_pct <= -self.margin_trigger:
                if self.margin_mode and data['adds_count'] < self.max_adds:
                    self.add_margin(symbol, current_price, leveraged_pnl_pct)
                    continue 
                
                if leveraged_pnl_pct <= -self.stop_loss:
                    sell = True; reason = f"🛑 STOP OUT (%{leveraged_pnl_pct:.2f})"
            ...
            if sell:
                ...
                return_amount = data['margin'] + pnl_amount
                if return_amount < 0: return_amount = 0 
                
                state['balance'] += return_amount
                self.log(f"OTO ÇIKIŞ: {symbol} | {reason} | Net PnL: ${pnl_amount:.2f}")
    ...
    def execute_buy(self, symbol, price, reason, total_equity):
        ...
        position_size_usdt = margin_amount * self.leverage
        amount_coins = position_size_usdt / price
        ...
        self.log(f"LONG AÇILDI ({self.leverage}x): {symbol} | Marjin: ${margin_amount:.1f}")
...
def analiz_motoru(symbol, current_data):
    ...
    if rvol_now > 5.0 and abs(price_chg) < 0.5: return "🐋 BALİNA GİRİŞİ", "bullish"
    
    if price_chg > 0 and rvol_now < 0.8: return "⚠️ ZAYIF YÜKSELİŞ", "bearish"
...
# ================= 4. ARAYÜZ & LOOP =================
st.sidebar.header("⚙️ Futures Ayarları")
bot_active = st.sidebar.checkbox("🎰 FUTURES BOTU AKTİF", value=False)
exchange = st.sidebar.selectbox("Borsa", ["BINANCE", "MEXC"])
refresh_rate = st.sidebar.selectbox("Hız (Sn)", [10, 30, 60], index=1)

st.sidebar.divider()
st.sidebar.subheader("🔥 Kaldıraç & Risk")
bot.leverage = st.sidebar.slider("Kaldıraç (x)", 1, 20, 10)
bot.trade_pct = st.sidebar.slider("İşlem Başı Bakiye (%)", 1, 20, 5)

st.sidebar.divider()
st.sidebar.subheader("🛡️ Kurtarma (Marjin Ekleme)")
bot.margin_mode = st.sidebar.checkbox("Otomatik Ekleme Açık", value=True)
bot.margin_trigger = st.sidebar.number_input("Ekleme Tetikleyicisi (Zarar %)", value=15.0)
bot.add_amount_pct = st.sidebar.number_input("Ekleme Miktarı (%)", value=100.0)
bot.max_adds = st.sidebar.number_input("Max Ekleme Sayısı", value=3, step=1)

st.sidebar.divider()
bot.take_profit = st.sidebar.number_input("Kâr Al (TP % - Kaldıraçlı)", value=50.0)
bot.stop_loss = st.sidebar.number_input("Stop Loss (SL % - Nihai)", value=30.0)
min_vol_m = st.sidebar.number_input("Min Hacim ($M)", value=1)

st.title("🎰 AI Futures Bot: Kaldıraç & Kurtarma Modu")
col_b1, col_b2, col_b3 = st.columns(3)
state = st.session_state.bot_state
placeholder_metrics = st.empty()

tab1, tab2, tab3 = st.tabs(["📊 PİYASA", "💼 AÇIK POZİSYONLAR", "📜 İŞLEM GEÇMİŞİ"])
...
while True:
    ...
    if not df.empty:
        ...
        # Metrikler (Düzeltilmiş Kısım)
        with placeholder_metrics.container():
            c1, c2, c3 = st.columns(3)
            diff = total_equity - bot.initial_balance
            
            c1.metric("TOPLAM VARLIK (USDT)", f"${total_equity:.2f}", delta=f"{diff:.2f}")
            c2.metric("KULLANILABİLİR BAKİYE", f"${state['balance']:.2f}")
            
            # Hesaplamayı dışarı aldık
            margin_per_trade = total_equity * (bot.trade_pct / 100)
            power_val = margin_per_trade * bot.leverage
            
            c3.metric("GELECEK İŞLEM GÜCÜ", f"${power_val:.0f}", help=f"Marjin ${margin_per_trade:.0f} x {bot.leverage}")

        with placeholder.container():
            if bot_active:
                bot.check_portfolio(current_prices)

            df_view = df.sort_values(by='priceChangePercent', ascending=False).head(20)
            tech_list = []
            ...
            with tab2:
                if state['portfolio']:
                    cols = st.columns([1.5, 1, 1, 1, 1, 1.5, 1])
                    cols[0].markdown("**Coin**")
                    cols[1].markdown("**Giriş**")
                    cols[2].markdown("**Güncel**")
                    cols[3].markdown("**PnL %**")
                    cols[4].markdown("**PnL $**")
                    cols[5].markdown("**Durum**")
                    cols[6].markdown("**Kapat**")
                    st.divider()

                    for sym, dat in list(state['portfolio'].items()):
                        curr = current_prices.get(sym, dat['entry'])
                        
                        raw_chg = (curr - dat['entry']) / dat['entry']
                        lev_pct = raw_chg * dat['leverage'] * 100
                        
                        pos_val = dat['amount_coins'] * curr
                        entry_val = dat['amount_coins'] * dat['entry']
                        pnl_usd = pos_val - entry_val
                        
                        pnl_color = "green" if lev_pct >= 0 else "red"

                        with st.container():
                            cc = st.columns([1.5, 1, 1, 1, 1, 1.5, 1])
                            cc[0].markdown(f"**{sym}** ({dat['leverage']}x)")
                            cc[1].text(f"{dat['entry']:.4f}")
                            cc[2].text(f"{curr:.4f}")
                            cc[3].markdown(f":{pnl_color}[%{lev_pct:.2f}]")
                            cc[4].markdown(f":{pnl_color}[${pnl_usd:.2f}]")
                            
                            adds_info = f"Ekleme: {dat['adds_count']}/{bot.max_adds}"
                            cc[5].caption(adds_info)
                            
                            if cc[6].button(f"KAPAT", key=f"btn_{sym}"):
